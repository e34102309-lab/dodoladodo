...
def evaluate_reit_equity(raw: Mapping[str, Any]) -> IndustryModelEvaluation:
    ...
    gain = _coalesce_number(metrics.get("gain_on_property_sale_ttm_b"), 0.0)
    impairment = abs(_coalesce_number(metrics.get("real_estate_impairment_ttm_b"), 0.0))
    maintenance = abs(_number(metrics.get("maintenance_capex_b")))
    interest = abs(_number(metrics.get("interest_ttm_b")))
    tax = _coalesce_number(metrics.get("tax_ttm_b"), 0.0)
