            ...
            layers.append(
                {
                    ...
                    "eligible": sum(bool(row.get("Long_Term_Eligible")) for row in layer_rows),
...
function prompt(x){let themeText=tags(x).length?tags(x).join('、'):'無明確主題標籤',layerText=layerTags(x).length?layerTags(x).join('；'):'尚無層級標籤';return[`請以中長期價值投資角度研究 ${x.Ticker}，不要直接下買賣指令。`,`Quant 分數：${f(x.Long_Term_Score)}；品質：${f(x.Quality_Score)}；價值：${f(x.Value_Score)}；資本配置：${f(x.Capital_Allocation_Score)}。`,`主題標籤：${themeText}。受益層級：${layerText}。請判斷它是一階、二階或三階受益者，還是只是被題材蹭到。`,`Real FCF Yield：${f(x.Real_FCF_Yield_pct,2,'%')}；ICR：${f(x.ICR,2,'x')}；ROIC：${f(x.ROIC_pct,2,'%')}。`,'請用最新官方財報回答：','1. 三句話投資論點。','2. 最強反方論點。','3. thesis 失效條件。','4. 悲觀、基準、樂觀情境。','5. 營收、毛利、Real FCF 與資產負債表警訊。','6. 股數稀釋與管理層資本配置。','7. 與 QQQ/VOO 的重疊，以及額外持有理由。','8. 主題供應鏈位置、訂單能見度、瓶頸、二階受益是否已開始進財報，以及是否已反映在估值。','9. 尚無法確認的資料。'].join('\n')}
