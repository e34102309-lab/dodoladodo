...
<style>
:root{--bg:#07111f;--panel:#0d1b2d;--line:#29425f;--text:#eef5ff;--muted:#9fb2c9;--blue:#62adff;--green:#58d5a0;--yellow:#f3cb67}*{box-sizing:border-box}body{margin:0;background:linear-gradient(145deg,#07111f,#0a1b2e);color:var(--text);font-family:system-ui,"Noto Sans TC",sans-serif}.shell{width:min(1320px,calc(100% - 24px));margin:auto;padding:24px 0 56px}.panel{background:rgba(13,27,45,.96);border:1px solid var(--line);border-radius:18px;box-shadow:0 16px 44px #0005}.hero{padding:24px;display:flex;justify-content:space-between;gap:20px;align-items:end}.hero h1{margin:4px 0 8px;font-size:clamp(28px,4vw,46px)}.hero p,.muted{color:var(--muted)}.stats{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin:14px 0}.stat{padding:16px}.stat small{display:block;color:var(--muted)}.stat strong{font-size:27px}.tools{display:grid;grid-template-columns:2fr 1fr 1fr auto;gap:9px;padding:12px}input,select,button{font:inherit;border:1px solid var(--line);border-radius:11px;padding:10px;background:#091827;color:var(--text)}button{cursor:pointer}button:hover{border-color:var(--blue)}.watch{display:flex;gap:9px;align-items:center;padding:12px;margin-top:12px}.watch input{max-width:230px}.table{margin-top:12px;overflow:auto}table{width:100%;border-collapse:collapse}th,td{padding:12px;border-bottom:1px solid var(--line);text-align:left}th{font-size:12px;color:var(--muted)}tbody tr{cursor:pointer}tbody tr:hover{background:#17314b88}.badge{display:inline-block;padding:3px 7px;border:1px solid var(--line);border-radius:999px;font-size:12px}.good{color:var(--green)}.warn{color:var(--yellow)}dialog{width:min(960px,calc(100% - 20px));max-height:90vh;padding:0;background:#0a1728;color:var(--text);border:1px solid var(--line);border-radius:18px}dialog::backdrop{background:#0010}.head{position:sticky;top:0;background:#0a1728ee;padding:15px;display:flex;justify-content:space-between;border-bottom:1px solid var(--line)}.body{padding:18px}.grid{display:grid;grid-template-columns:repeat(3,1fr);gap:9px}.metric,.box{padding:12px;border:1px solid var(--line);border-radius:12px;background:var(--panel)}.metric small{display:block;color:var(--muted);margin-bottom:5px}.section{margin-top:18px}.actions{display:flex;gap:8px;flex-wrap:wrap}a{color:var(--blue)}.footer{text-align:center;color:var(--muted);font-size:12px;margin-top:18px}@media(max-width:760px){.hero{display:block}.stats,.grid{grid-template-columns:repeat(2,1fr)}.tools{grid-template-columns:1fr 1fr}.watch{align-items:stretch;flex-direction:column}.watch input{max-width:none}.optional{display:none}}@media(max-width:480px){.stats,.grid,.tools{grid-template-columns:1fr}}
</style></head><body><main class="shell">
<section class="hero panel"><div><span class="badge good">不依賴外部 AI</span><h1>Alpha Engine 長期價值研究台</h1><p>Mode C 整理數據與風險；最後判斷由你完成。</p></div><div class="muted">QQQ 40% · VOO 30% · 主動個股 0–30%<br>單股上限 3% · 單一主動產業上限 9%</div></section>
...
<section class="tools panel"><input id="search" placeholder="搜尋 ticker、產業或結論"><select id="view"><option value="all">全部結果</option><option value="shortlist">Shortlist</option><option value="eligible">研究合格</option><option value="watch">我的追蹤</option></select><select id="min"><option value="0">不限分數</option><option>60</option><option>70</option><option>75</option><option>80</option></select><button id="refresh">重新整理</button></section>
...
<section class="table panel"><table><thead><tr><th>排名</th><th>Ticker</th><th>分數</th><th>結論</th><th class="optional">產業</th><th class="optional">Real FCF Yield</th><th>追蹤</th></tr></thead><tbody id="rows"></tbody></table><p id="empty" class="muted" style="padding:20px" hidden>沒有符合條件的股票。</p></section><div id="updated" class="footer"></div></main>
...
const data=JSON.parse(document.querySelector('#payload').textContent),stocks=data.stocks||[],map=new Map(stocks.map(x=>[x.Ticker,x])),key='alphaEngineWatchlistV1';let watch=load();
const $=s=>document.querySelector(s),e=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c])),n=v=>{v=Number(v);return Number.isFinite(v)?v:null},f=(v,d=2,s='')=>n(v)===null?'N/A':n(v).toFixed(d)+s,yes=v=>v===true||String(v).toLowerCase()==='true',norm=v=>String(v||'').toUpperCase().replace(/[^A-Z0-9.-]/g,'').slice(0,10);
...
function visible(){let q=$('#search').value.toLowerCase(),v=$('#view').value,m=Number($('#min').value);let out=stocks.filter(x=>{let hay=[x.Ticker,x.Sector,x.Industry,x.Status,x.Verdict].join(' ').toLowerCase();return(!q||hay.includes(q))&&(n(x.Long_Term_Score)??-1)>=m&&(v!=='shortlist'||yes(x.IsShortlist))&&(v!=='eligible'||yes(x.Long_Term_Eligible))&&(v!=='watch'||watch.has(x.Ticker))});if(v==='watch')for(let t of watch)if(!map.has(t)&&(!q||t.toLowerCase().includes(q)))out.push({Ticker:t,Status:'尚未在本次篩選資料'});return out}
function render(){let out=visible();$('#rows').innerHTML=out.map(x=>`<tr data-t="${e(x.Ticker)}"><td>${x.Rank||'-'}</td><td><b>${e(x.Ticker)}</b> ${yes(x.IsShortlist)?'<span class="badge good">Shortlist</span>':''}</td><td>${f(x.Long_Term_Score)}</td><td>${e(x.Verdict||x.Status||'待查')}</td><td class="optional">${e(x.Sector||x.Industry||'N/A')}</td><td class="optional">${f(x.Real_FCF_Yield_pct,2,'%')}</td><td><button data-w="${e(x.Ticker)}">${watch.has(x.Ticker)?'移除':'加入'}</button></td></tr>`).join('');$('#empty').hidden=out.length>0;document.querySelectorAll('tr[data-t]').forEach(r=>r.onclick=a=>{if(!a.target.dataset.w)openDetail(r.dataset.t)});document.querySelectorAll('[data-w]').forEach(b=>b.onclick=a=>{a.stopPropagation();toggle(b.dataset.w)})}
...
function prompt(x){return[`請以中長期價值投資角度研究 ${x.Ticker}，不要直接下買賣指令。`,`Quant 分數：${f(x.Long_Term_Score)}；品質：${f(x.Quality_Score)}；價值：${f(x.Value_Score)}；資本配置：${f(x.Capital_Allocation_Score)}。`,`Real FCF Yield：${f(x.Real_FCF_Yield_pct,2,'%')}；ICR：${f(x.ICR,2,'x')}；ROIC：${f(x.ROIC_pct,2,'%')}。`,'請用最新官方財報回答：','1. 三句話投資論點。','2. 最強反方論點。','3. thesis 失效條件。','4. 悲觀、基準、樂觀情境。','5. 營收、毛利、Real FCF 與資產負債表警訊。','6. 股數稀釋與管理層資本配置。','7. 與 QQQ/VOO 的重疊，以及額外持有理由。','8. 尚無法確認的資料。'].join('\n')}
...
function openDetail(t){let x=map.get(t)||{Ticker:t,Status:'尚未在本次篩選資料'},s=sec(x);$('#detailTitle').textContent=x.Ticker;$('#detailBody').innerHTML=`<div class="actions"><button id="dw">${watch.has(t)?'移除追蹤':'加入追蹤'}</button><button id="cp">複製 AI 研究提示</button>${s?`<a target="_blank" rel="noopener" href="${s}">SEC 官方財報</a>`:''}<a target="_blank" rel="noopener" href="${yahoo(t,'financials')}">財務報表頁</a><a target="_blank" rel="noopener" href="${yahoo(t)}">市場資料頁</a></div><div class="section"><h3>模型結論</h3><div class="box">${e(x.Research_Action||x.Verdict||x.Status||'待查')}</div></div><div class="section"><h3>財務與風險指標</h3><div class="grid">${fields.map(a=>`<div class="metric"><small>${a[0]}</small><b>${f(x[a[1]],2,a[2]||'')}</b></div>`).join('')}</div></div><div class="section"><h3>資料品質與待查事項</h3><div class="box">${e(x.GM_Diagnosis||'')}<br>${e(x.Data_Quality_Flags||'無資料品質警示')}<br>${e(x.Agent_Tasks||'請從 SEC 官方財報開始查核。')}</div></div>`;$('#dw').onclick=()=>toggle(t);$('#cp').onclick=()=>copy(prompt(x));if(!$('#detail').open)$('#detail').showModal()}
$('#total').textContent=data.stats.total;$('#eligible').textContent=data.stats.eligible;$('#shortlist').textContent=data.stats.shortlist;$('#updated').textContent='資料更新：'+new Date(data.generated_at).toLocaleString('zh-TW')+' · 本網站僅供研究，不是投資建議。';['#search','#view','#min'].forEach(s=>$(s).oninput=render);$('#refresh').onclick=render;$('#add').onclick=()=>{let t=norm($('#addTicker').value);if(t){watch.add(t);$('#addTicker').value='';save();render();openDetail(t)}};$('#addTicker').onkeydown=a=>{if(a.key==='Enter')$('#add').click()};$('#export').onclick=()=>{let text=[...watch].sort().join('\n'),a=document.createElement('a');a.href=URL.createObjectURL(new Blob([text+(text?'\n':'')],{type:'text/plain;charset=utf-8'}));a.download='alpha-engine-watchlist.txt';a.click();URL.revokeObjectURL(a.href)};$('#close').onclick=()=>$('#detail').close();save();render();
</script></body></html>'''
