"""
=============================================================================
GROWTH SATELLITE PIPELINE v3.5 (動態環境校正與 SPY 物理隔離版)
=============================================================================
v3.5 改動摘要：
1. ★[架構] SPY 大盤基準獨立抓取，徹底解決 yf.download 併發導致 MultiIndex 遺失 SPY 資料的靜默失效 Bug。
2. ★[邏輯] 實裝動態環境動能測試 (Dynamic Regime-Adjusted Momentum)，讓成長股的絕對動能底線隨大盤 Beta 浮動。
3. ★[數據] 擴充 SECDataDistiller 標籤庫 (GrossMargin, SalesRevenueGoodsNet 等)，拯救錯殺標的。
=============================================================================
"""
import random
import requests
...
REV_GROWTH_MIN          = 12.0         # 營收增速底線
GROSS_MARGIN_MIN        = 0.35         # 毛利率底線
RULE_OF_40_MIN          = 25.0         # 真實 Rule of 40 底線
MOMENTUM_MIN            = -5.0         # ★絕對動能底線 (現為多頭基準，熊市會動態放寬)
RELATIVE_MOMENTUM_MIN   = -10.0        # ★相對 SPY 動能底線
FCF_BURN_FLOOR          = -40.0        # 燒錢深度紅線
CASH_RUNWAY_MIN_YRS     = 2.0          # 燒錢公司現金跑道底線
DILUTION_MAX_YOY        = 0.10         # 流通股年增上限

# === v3 新增紅線 (v3.3 微調) ===
PCT_FROM_52W_HIGH_MIN   = -0.45        # 距 52 週高點
GROSS_MARGIN_TREND_MIN  = -0.03        # 毛利率 YoY 衰退
EV_SALES_MAX            = 30.0         # EV/Sales 極端紅線

# === v3.3 新增：市值地板 ===
MIN_MARKET_CAP_B        = 0.5          # 市值地板
...
def pre_fetch_all_info(tickers: List[str]):
    global _INFO_CACHE
    logger.info(f"[v3.5] 主線程批量抓取 {len(tickers)} 檔 yf.info（單線程，避封鎖）...")
    success = 0
    ...
    logger.info(f"[v3.5] info 批量完成：成功 {success}/{len(tickers)}")
...
class SECDataDistiller:
    def __init__(self, email: str):
        ...
        # ★ v3.5 擴充 XBRL 標籤庫，減少 31 檔缺失的漏網之魚
        self.config = {
...
def run_growth_satellite_pipeline(ticker: str, cik: str, email: str,
    ...
    try:
        ...
        # ★ v3.5 動態環境動能校正 (Dynamic Regime-Adjusted Momentum)
        spy_mom = (mom_12m - rel_mom) if rel_mom is not None else 0.0
        
        # 絕對動能防線：容許成長股承受大盤 1.5 倍的回撤
        ...
        debt_lt = abs(sec.get_latest_annual(df_debt))
        debt_st = abs(sec.get_latest_annual(df_std))
        ...
        if real_fcf < 0 and cash_total > 0:
            annual_outflow = abs(real_fcf) + debt_st
        ...
        return {
            ...
            'Total_Debt(B)':         round(debt_total, 3),
            'ShortTerm_Debt(B)':     round(debt_st, 3),
...
def send_email_report(df: pd.DataFrame, receiver_email: str):
    ...
    msg = EmailMessage()
    msg['Subject'] = f"[🚀 成長衛星 v3.5] Alpha 報表 - {datetime.now().strftime('%Y-%m-%d %H:%M')}"
    msg['From'], msg['To'] = sender_email, receiver_email

    content = (
        "總工程師您好：\n\n【成長衛星 v3.5 動態校正與SPY隔離版 掃描完成】\n"
        f"核心：真實 Rule40>{RULE_OF_40_MIN}、營收增長>{REV_GROWTH_MIN}%、動能隨大盤Beta動態校正\n"
        f"v3.5 修復：SPY獨立抓取防死鎖、擴充SEC標籤庫\n"
        + "-" * 70 + "\n\n"
    )
    ...
    if not df.empty:
        msg.add_attachment(df.to_csv(index=False, encoding='utf-8-sig').encode('utf-8-sig'), 
                           maintype='text', subtype='csv', filename=f'Growth_v3_5_{datetime.now().strftime("%Y%m%d")}.csv')
    try:
...
if __name__ == "__main__":
    ...
    try:
        ...
        # ★ v3.5 關鍵修復 1：獨立抓取 SPY 基準，徹底避免 MultiIndex 併發遺失
        ...
        # ── 2. 一次性批量下載候選股資料 (已移除 SPY，讓 SPY 保持乾淨獨立) ──
