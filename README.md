...
- GitHub Actions 在工作日每日使用最近一次完整 universe 跑 Mode C；每月 1 日才重跑全市場初篩。push/PR 不重跑耗時初篩。
...
本機產生的網站位於 `public/index.html`。`qualified_universe.csv` 可由本機手動更新；GitHub Actions 在工作日每日以現有 universe 更新 Mode C，每月 1 日才重跑全市場初篩並提交新的 `qualified_universe.csv`。PR 只跑品質檢查。
