"""
=============================================================================
V11.2 SBC FIX & CONCURRENCY EDITION (核心持股管線 - 結構性 bug 修復版)
=============================================================================
v11.2 改動摘要 (vs v11.1)：
1. ★[嚴重] EBITDA 壓力測試一致化：
   - 原本 real_fcf = OCF - capex - sbc 已經把 sbc 視為實質成本，
     卻在 stress test 用 ebitda = ebit + dna + sbc 把它加回來，內部矛盾。
   - v11.2 改為 ebitda = ebit + dna，剔除 SBC 的會計幻覺，壓力測試更保守。
2. ★[嚴重] 併發/yf.Ticker 衝突修正：
   - worker 線程內呼叫 yf.Ticker(t).info 觸發限流，自爆 _BULK_MARKET_DATA 初衷。
   - 新增 pre_fetch_all_info()：啟動時主線程批量抓一次。
   - safe_yf_info() 改純讀 _INFO_CACHE，worker 線程零網路呼叫。
=============================================================================
"""
import random
import requests
...
def pre_fetch_all_info(tickers: List[str]):
    ...
    v11.2 關鍵修正：之前 safe_yf_info 在 worker 線程內 new yf.Ticker(t).info，
    高頻併發會觸發 401/429，自爆 _BULK_MARKET_DATA 的初衷。
    現在改為啟動時單線程順序抓一次，全部存進 _INFO_CACHE，worker 只讀字典。
    """
    global _INFO_CACHE
    logger.info(f"[v11.2] 主線程批量抓取 {len(tickers)} 檔 yf.info（單線程，避封鎖）...")
    success = 0
    ...
    logger.info(f"[v11.2] info 批量完成：成功 {success}/{len(tickers)}")
...
def send_email_report(df: pd.DataFrame, receiver_email: str, trend_report: str):
    ...
    if not sender_email or not sender_pwd:
        ...
        return
    msg = EmailMessage()
    msg['Subject'] = f"[V11.2 核心持股] Alpha 報表 - {datetime.now().strftime('%Y-%m-%d %H:%M')}"
    msg['From'] = sender_email
    msg['To'] = receiver_email
    content = f"總工程師您好：\n\n【全域監測】\n{trend_report}\n" + "-" * 60
    content += "\n[v11.2 結構性修復版] EBITDA 壓力測試剔除 SBC 加回；worker 線程零 yf.Ticker 呼叫。\n\n"
    if df.empty:
        content += "今日無通關標的。"
    else:
    ...
    if not df.empty:
        msg.add_attachment(
            ...
            filename=f'V11.2_Alpha_{datetime.now().strftime("%Y%m%d")}.csv'
        )
    try:
